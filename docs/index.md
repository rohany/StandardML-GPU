...
implementations of parenthesis matching, how to 
tell whether a sequence of parentheses ( where ( = 1 and ) = -1 ) is matched or not.
Using Standard ML, and with our GPU library, the algorithms compute a prefix sum
across the input sequence, and then check to make sure element at the end is a 0, and
...
With almost no changes to the code, this algorithm runs almost 20 times faster than the standard 
sequence implementation. Other than referencing our extended GPUSeq structure and swapping out
higher order functions with references to functions inside a Lambdas structure, this code will 
now compile and run on the GPU. 
...
> This plot is interactive - click on the legend to show or hide traces
...
In paralell to this project, research at CMU has been working on runing Standard ML in parallel on multicore CPU's.
These researchers How does our implementation fare against a well written, parallel `SEQUENCE` implementation for that research?
